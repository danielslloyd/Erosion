...
function generateGoodPoints(n, extent) {
    ...
    return improvePoints(pts, 4, extent);
}
...
function erosionRate(h,v) {
    var flux = getFlux(h);
    var slope = getSlope(h);
    var newh = zero(h.mesh);
    if (v == 1) {
        for (var i = 0; i < h.length; i++) {
            // var total = 1000 * river + creep;//original
            // var total = 100*Math.pow(flux[i],0.5)*Math.pow(slope[i],1.1)+0.3*Math.pow(slope[i],1.1); //working
            var total = 100*Math.pow(flux[i],0.5)*Math.pow(slope[i],1.1)+0.3*Math.pow(slope[i],1.1);
            total = h[i] < 0.01 ? total*0.05 : total;
            total = total > 200 ? 200 : total; //200
            newh[i] = total;
        }
    } else {
        for (var i = 0; i < h.length; i++) {
...
function erode(h, amount, v) {
    var er = erosionRate(h,v);
    var newh = zero(h.mesh);
    var maxr = d3.max(er);
    for (var i = 0; i < h.length; i++) {
...
function doErosion(h, amount, n, v) {
    n = n || 1;
    h = fillSinks(h);
    for (var i = 0; i < n; i++) {
        h = erode(h, amount, v);
...
function cleanCoast(h, iters,ev) {
...
function cityScore(h, cities) {
    ...
    for (var i = 0; i < h.length; i++) {
        ...
        var nbs = neighbours(score.mesh, i);
        //var nbh = neighbours(h.mesh, i);
        var nb = 0;
        var minh = 1000000;
        for (var j = 0; j < nbs.length; j++) {
            if (h[nbs[j]] < minh) {minh = h[nbs[j]]}
            if (fluxH[nbs[j]] > nb && fluxH[nbs[j]] < fluxH[i]) {
               nb = fluxH[nbs[j]];
            }
        }
        score[i] = nb < fluxH[i]*0.7 ? score[i] + 2*fluxH[i] : score[i];   
        
        score[i] += 0.005 / (1e-9 + Math.abs(h.mesh.vxs[i][0]) - h.mesh.extent.width/2)
        score[i] += 0.005 / (1e-9 + Math.abs(h.mesh.vxs[i][1]) - h.mesh.extent.height/2)
        for (var j = 0; j < cities.length; j++) {
            score[i] -= 0.02 / Math.pow((distance(h.mesh, cities[j], i) + 1e-9),0.8);
        }
        score[i] = minh <= 0 ? score[i] + 4*fluxH[i] : score[i];
    }
    return score;
}
