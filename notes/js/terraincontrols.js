...
var primH = zero(generateGoodMesh(16384));
...
function generateUneroded() {
    var mesh = generateGoodMesh(16384);
    var h = add(slope(mesh, randomVector(4)),
                cone(mesh, runif(-1, 1)),
                mountains(mesh, 500,0.01));
    h = peaky(h);
    h = fillSinks(h);
    h = setSeaLevel(h, 0.5);
    return h;
...
erodeDiv.append("button")
    ...
    .on("click", function () {
        ...
        //erodeH = doErosion(erodeH, 0.1,0);
        erodeH = setSeaLevel(erodeH, 0.4);
        //erodeH = cleanCoast(erodeH, 3,0.8);
        //erodeH = cleanCoast(erodeH, 3,0.6);
        erodeH = cleanCoast(erodeH, 3,0);
...
physDiv.append("button")
    .text("Generate random heightmap")
    .on("click", function () {
        physH = generateCoast({npts:16384, extent:defaultExtent});
        physDraw();
    });
...
function newCityRender(h) {
    h = h || generateCoast({npts:16384, extent: defaultExtent});
    return {
